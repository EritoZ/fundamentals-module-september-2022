...
while a_string.count(pattern) >= 2 and pattern:
    ...
    a_string = a_string[:index_first_occurrence] \
               + a_string[index_first_occurrence + len(pattern):index_second_occurrence] \
               + a_string[index_second_occurrence + len(pattern):]
